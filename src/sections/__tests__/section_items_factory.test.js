  ...
  it("test SectionFactory", () => {
    ...
    let wrapper = shallow(
      <SectionFactory
        ...
        section_id={2}
        section_handler={section_handler}
        section_keys={["wall", "window"]}
        section_key="wall"
      />
    );
    expect(wrapper.html()).toEqual(
      '<select><option> wall </option><option> window </option></select><label>desc<input type="string" name="desc" id="0"/></label><label>height<input type="number" name="height" id="1"/></label>'
    );
  });
